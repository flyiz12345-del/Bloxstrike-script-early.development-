...
-- // RESOLUTION GUARD: Wait for Loadstring to settle
task.wait(0.5) 
local ViewSize = C.ViewportSize

-- // UI BASE (Fixed Pixel Offsets to prevent PC Flip)
...
local function CreatePanel(y)
    ...
    Instance.new("UIListLayout",p).Padding,p.UIListLayout.HorizontalAlignment=UDim.new(0,5),1
    return p
end
...
local function Ico(sym,panel,y)
    ...
    b.MouseButton1Click:Connect(function() local v=not panel.Visible AimP.Visible,VisP.Visible,RageP.Visible=false,false,false panel.Visible=v end)
end
