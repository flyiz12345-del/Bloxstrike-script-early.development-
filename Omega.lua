...
task.wait(0.5)local G=Instance.new("ScreenGui",LP.PlayerGui)G.Name="OmegaV26"G.IgnoreGuiInset=true
...
local function Shoot()local v=game:GetService("VirtualInputManager")if v then v:SendMouseButtonEvent(0,0,0,true,game,0)task.wait(0.02)v:SendMouseButtonEvent(0,0,0,false,game,0)end end
...
if t then 
    if _G.SA then C.CFrame = CFrame.lookAt(C.CFrame.Position, t.Position) end -- Pure Silent (Instant Snap)
    if _G.A and not _G.SA then C.CFrame=C.CFrame:Lerp(CFrame.lookAt(C.CFrame.Position,t.Position),1/_G.Sm) end
